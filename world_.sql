...
-- Dumping data for table world.city: ~4,046 rows (approximately)
DELETE FROM `city`;
...
-- Dumping data for table world.user: ~0 rows (approximately)
DELETE FROM `user`;
...
CREATE TABLE IF NOT EXISTS `userchoice` (
  ...
  KEY `FK_CityId` (`cityID`) USING BTREE,
  CONSTRAINT `FK_CityId` FOREIGN KEY (`cityID`) REFERENCES `city` (`ID`),
  CONSTRAINT `FK_UserId` FOREIGN KEY (`userID`) REFERENCES `user` (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Dumping data for table world.userchoice: ~0 rows (approximately)
